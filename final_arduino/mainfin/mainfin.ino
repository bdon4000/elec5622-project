...
void loop() 
{
  Pulse_Allot();
  GPS_Allot();
  if(millis()-time_limit1>5000)//write data in sd card and calculate calories every 5 second.
  {
    GPS_SD_Write();
    PulseSensor_SD_Write();
    hall_SD_Write();
    time_limit1 = millis();
    calories=calories+((BPM/195-50)/10)*3.425/750;
  ...
  tft.fillRect(0,345,174,43,WHITE);
  tft.print(capdence);
  tft.setCursor(0,84);
  tft.println(speed1);
  tft.setCursor(0,150);
